...
export g, ErrAboveModelTop, findLayer, calcDeltaH, ASME, calcDeltaHPrecomputed, ASMEPrecomputed
  ...
  function calcDeltaH(stackLayer::Int, temperature, windSpeed, sClass, s1::Vector{Float64}, stackHeight, stackTemp, stackVel, stackDiam::Float64)
    deltaH = 0. # Plume rise, (m).

    airTemp = temperature[stackLayer]
    windSpd = windSpeed[stackLayer]

    if (stackTemp-airTemp) < 50. && stackVel > windSpd && stackVel > 10. 
      ...
      if sClass[stackLayer] > 0.5  # stable conditions
        ...
        deltaH = 29. * ((F/s1[stackLayer]) ^ 0.333333333) / (windSpd ^ 0.333333333)

      else # unstable conditions

        deltaH = 7.4 * ((F*(stackHeight ^ 2.)) ^ 0.333333333) / windSpd
      end
    end
    if isnan(deltaH)
      throw(ErrorException(string("plume height == NaN ",
  ...
  function ASME(stackHeight, stackDiam, stackTemp, stackVel::Float64, layerHeights, temperature, windSpeed, sClass, s1::Vector{Float64})
    stackLayer = findLayer(layerHeights, stackHeight)
    deltaH = calcDeltaH(stackLayer, temperature, windSpeed, sClass, s1, stackHeight, stackTemp, stackVel, stackDiam)
    print(deltaH)
  ...
  function calcDeltaHPrecomputed(stackLayer::Int, temperature, windSpeed, sClass, s1::Vector{Float64}, stackHeight, stackTemp, stackVel, stackDiam::Float64, windSpeedMinusOnePointFour, windSpeedMinusThird, windSpeedInverse::Vector{Float64})
    ...
    if (stackTemp-airTemp) < 50. && stackVel > windSpd && stackVel > 10.
      ...
      if isnan(deltaH)
        throw(ErrorException(string("plumerise: momentum-dominated deltaH is NaN. ",  "stackDiam: $(stackDiam), stackVel: $(stackVel), windSpeedMinusOnePointFour: $(windSpeedMinusOnePointFour[stackLayer])")))
        return deltaH
      end
      ...
      if sClass[stackLayer] > 0.5 && s1[stackLayer] != 0 && F > 0 # stable conditions
        ...
        deltaH = 29. * ((F/s1[stackLayer]) ^ 0.333333333) * windSpeedMinusThird[stackLayer]

        if isnan(deltaH)
          throw(ErrorException(string("plumerise: stable bouyancy-dominated deltaH is NaN. ", "F: $(F), s1: $(s1[stackLayer]), windSpeedMinusThird: $(windSpeedMinusThird[stackLayer])")))
          return deltaH
        end

      elseif F > 0.  # unstable conditions

        deltaH = 7.4 * ((F*(stackHeight ^ 2.)) ^ 0.333333333) * windSpeedInverse[stackLayer]

        if isnan(deltaH)
          throw(ErrorException(string("plumerise: unstable bouyancy-dominated deltaH is NaN. ", "F: $(F), stackHeight: $(stackHeight), windSpeedInverse: $(windSpeedInverse[stackLayer])")))
          return deltaH
        end
      else
  ...
  # Uses the plume rise calculation: ASME (1973), as described in Sienfeld and Pandis,
  # ``Atmospheric Chemistry and Physics - From Air Pollution to Climate Change
  function ASMEPrecomputed(stackHeight, stackDiam, stackTemp,
    ...
    stackLayer = findLayer(layerHeights, stackHeight)
    deltaH = calcDeltaHPrecomputed(stackLayer, temperature, windSpeed, sClass,
